classdef MatchTeam1DCPWA < LSIPMinCuttingPlaneAlgo
    % Class for matching for teams problems with one-dimensional marginals, a quality space which is a two-dimensional polytope, and 
    % cost functions that have the form c_i(x_i, z) = l_i(x_i - s' * z), where l_i is a continuous piece-wise affine (CPWA) function. 
    % The problem is solved via parametrizing the transfer functions on the quality space.
    ...
    methods(Access = public)
        function obj = MatchTeam1DCPWA(marginals, costfuncs, quality_vertices, quality_testfuncs, varargin)
            % Constructor method
            % Inputs:
            %   marginals: cell array containing objects of ProbMeas1DInterval
            %   costfuncs: cell array containing structs with fields weights, knots, and values
            %   quality_vertices: two-column matrix containing the vertices of the quality space
            %   quality_testfuncs: cell array containing the inputs to the function obj.setQualitySimplicialTestFuncs

            q_vert_ccorder = convhull(quality_vertices(:, 1), quality_vertices(:, 2), 'Simplify', true);
            q_vert_cc1 = quality_vertices(q_vert_ccorder(1:end - 1), :);
            q_vert_cc2 = quality_vertices(q_vert_ccorder(2:end), :);

            % compute the weights and the intercepts of the hyperplanes characterizing the quality space
            hp_w = [q_vert_cc2(:, 2) - q_vert_cc1(:, 2), q_vert_cc1(:, 1) - q_vert_cc2(:, 1)];
            hp_b = q_vert_cc2(:, 2) .* q_vert_cc1(:, 1) - q_vert_cc1(:, 2) .* q_vert_cc2(:, 1);
            ...
            % build the representation of the quality space used by the constructor of MatchTeam1DCPWA
            quality_repr = struct;
            quality_repr.dim = 2;
            ...
            obj.Quality = struct;
            obj.Quality.Dim = quality_repr.dim;
            obj.Quality.AuxVarNum = quality_repr.aux_num;

            % build the representation of the quality space
            if ~isfield(quality_repr, 'eq_A') || isempty(quality_repr.eq_A) ...
                    || ~isfield(quality_repr, 'eq_rhs') || isempty(quality_repr.eq_rhs)
                quality_repr.eq_A = sparse(0, quality_repr.dim + quality_repr.aux_num);
                quality_repr.eq_rhs = zeros(0, 1);
            end

            if ~isfield(quality_repr, 'ineq_A') || isempty(quality_repr.ineq_A) ...
                    || ~isfield(quality_repr, 'ineq_rhs') || isempty(quality_repr.ineq_rhs)
                quality_repr.ineq_A = sparse(0, quality_repr.dim + quality_repr.aux_num);
                quality_repr.ineq_rhs = zeros(0, 1);
            end

            if ~isfield(quality_repr, 'lb') || isempty(quality_repr.lb)
                quality_repr.lb = -inf(quality_repr.dim + quality_repr.aux_num, 1);
            end

            if ~isfield(quality_repr, 'ub') || isempty(quality_repr.ub)
                quality_repr.ub = inf(quality_repr.dim + quality_repr.aux_num, 1);
            end

            ineq_num = length(quality_repr.ineq_rhs);
            eq_num = length(quality_repr.eq_rhs);

            obj.Quality.GurobiModel = struct;
            obj.Quality.GurobiModel.A = sparse([quality_repr.eq_A; quality_repr.ineq_A]);
            obj.Quality.GurobiModel.rhs = [quality_repr.eq_rhs; quality_repr.ineq_rhs];
            obj.Quality.GurobiModel.sense = [repmat('=', eq_num, 1); repmat('<', ineq_num, 1)];
            obj.Quality.GurobiModel.lb = quality_repr.lb;
            obj.Quality.GurobiModel.ub = quality_repr.ub;
            ...
            for var_id = 1:obj.Quality.Dim
                for dir_sign = [-1, 1]
                    ...
                    check_model.obj = zeros(obj.Quality.Dim + obj.Quality.AuxVarNum, 1);
            ...
            for marg_id = 1:marg_num
                ...
                % check the inputs 
                assert(length(cf.weights) == obj.Quality.Dim, 'weights in the cost function are mis-specified');
                assert(all(obj.CostFuncs{marg_id}.KnotDiff > 0), 'knots in the cost function are not in ascending order');

                obj.CostFuncs{marg_id}.Values = cf.values;
                obj.CostFuncs{marg_id}.ValueDiff = diff(cf.values);

                % Lipschitz constant wrt change in the first argument (the input from the support of the marginal)
                obj.CostFuncs{marg_id}.LipschitzConstMarg = max(abs(obj.CostFuncs{marg_id}.ValueDiff ...
                    ./ obj.CostFuncs{marg_id}.KnotDiff));

                % Lipschitz constant wrt change in the second argument (the input from the quality space)
                obj.CostFuncs{marg_id}.LipschitzConstQuality = obj.CostFuncs{marg_id}.LipschitzConstMarg ...
                    * sqrt(cf.weights' * cf.weights);

                % check the domain of the CPWA function
                check_model = obj.Quality.GurobiModel;
                check_model.obj = [cf.weights; zeros(obj.Quality.AuxVarNum, 1)];
                check_model.modelsense = 'min';
                ...
                assert(obj.CostFuncs{marg_id}.Knots(end) >= obj.Marginals{marg_id}.Supp.UpperBound - check_result.objval, ...
                ...
                assert(obj.CostFuncs{marg_id}.Knots(1) <= obj.Marginals{marg_id}.Supp.LowerBound - check_result.objval, ...
                    'domain of the CPWA function is too small');

                obj.Storage.MarginalLowerBounds(marg_id) = obj.Marginals{marg_id}.Supp.LowerBound;
                obj.Storage.MarginalUpperBounds(marg_id) = obj.Marginals{marg_id}.Supp.UpperBound;
            end

            % set up the gurobi model for computing the minimum cost over the quality space
            ...
            quality_indices = (1:(obj.Quality.Dim + obj.Quality.AuxVarNum))';
            
            % variables between 0 and 1 that are used to interpolate within the intervals
            ...
            for marg_id = 1:marg_num
                ...
                zeta_indices{marg_id} = var_counter + (1:costfunc_knot_num - 1)';
                var_counter = var_counter + costfunc_knot_num - 1;
                iota_indices{marg_id} = var_counter + (1:costfunc_knot_num - 2)';
                var_counter = var_counter + costfunc_knot_num - 2;

                lb_cell{marg_id} = [zeros(costfunc_knot_num - 1, 1); -inf(costfunc_knot_num - 2, 1)];
                ub_cell{marg_id} = [1; inf(costfunc_knot_num - 2, 1); inf(costfunc_knot_num - 2, 1)];
                vtype_cell{marg_id} = [repmat('C', costfunc_knot_num - 1, 1); repmat('B', costfunc_knot_num - 2, 1)];

                obj_con = obj_con + costfunc_vals(1);
                obj_cell{marg_id} = [diff(costfunc_vals); zeros(costfunc_knot_num - 2, 1)];
            end

            quality_model = obj.Quality.GurobiModel;
            min_cost_model.objcon = obj_con;
            min_cost_model.obj = [zeros(obj.Quality.Dim + obj.Quality.AuxVarNum, 1); vertcat(obj_cell{:})];
            min_cost_model.lb = [quality_model.lb; vertcat(lb_cell{:})];
            min_cost_model.ub = [quality_model.ub; vertcat(ub_cell{:})];
            min_cost_model.vtype = [repmat('C', obj.Quality.Dim + obj.Quality.AuxVarNum, 1); vertcat(vtype_cell{:})];
            ...
            for marg_id = 1:marg_num
                ...
                % constraint: iota_{i,t} - zeta_{i,t} <= 0 and constraint: iota_{i,t} - zeta_{i, t+1} >= 0
                A_ordering_r = repelem((1:(costfunc_knot_num - 2) * 2)', 2, 1);
                A_ordering_c_mat = [iota_indices{marg_id}'; zeta_indices{marg_id}(1:end - 1)'; ...
                    iota_indices{marg_id}'; zeta_indices{marg_id}(2:end)'];
                A_ordering_c = A_ordering_c_mat(:);
                A_ordering_v = repmat([1; -1; 1; -1], costfunc_knot_num - 2, 1);
                A_ordering = sparse(A_ordering_r, A_ordering_c, A_ordering_v, 2 * (costfunc_knot_num - 2), var_counter);
                rhs_ordering = zeros(2 * (costfunc_knot_num - 2), 1);
                sense_ordering = repmat(['<'; '>'], costfunc_knot_num - 2, 1);

                % constraint linking zeta and the inputs
                A_link = sparse(ones(costfunc_knot_num - 1 + obj.Quality.Dim, 1), ...
                    [zeta_indices{marg_id}; quality_indices(1:obj.Quality.Dim)], ...
                    [diff(costfunc_knots); obj.CostFuncs{marg_id}.Weights], 1, var_counter);
                rhs_link = -costfunc_knots(1);
                sense_link = '=';

                rhs_input_indices(marg_id) = constr_counter + 2 * costfunc_knot_num - 3;
            ...
            min_cost_model.A = [vertcat(A_cell{:}); ...
                [quality_model.A, sparse(size(quality_model.A, 1), var_counter - obj.Quality.Dim - obj.Quality.AuxVarNum)]];
            min_cost_model.rhs = [vertcat(rhs_cell{:}); quality_model.rhs];
            min_cost_model.sense = [vertcat(sense_cell{:}); quality_model.sense];
            ...
            % set the default option for the formulation of global minimization problem (options are 'LOG_DLOG' and 'INC_CC')
            if ~isfield(obj.Options, 'global_formulation') || isempty(obj.Options.global_formulation)
                obj.Options.global_formulation = 'LOG_DLOG';
            end

            % set the default options for whether to use parfor when executing the global oracle
            if ~isfield(obj.Options, 'global_parfor') || isempty(obj.Options.global_parfor)
                obj.Options.global_parfor = false;
            end
            ...
            % set the default options for the minimum violation for a new cut to be introduced in the sense that a new constraint is
            % added to subsequent LP relaxations only if its current violation < -obj.Options.new_cut_thres
            if ~isfield(obj.Options, 'new_cut_thres') || isempty(obj.Options.new_cut_thres)
                obj.Options.new_cut_thres = 0;
            end

            % set the default option for sanitizing the entries in the inequality constraints
            if ~isfield(obj.Options, 'sanitation_threshold') || isempty(obj.Options.sanitation_threshold)
            ...
            if isfield(obj.Quality, 'SimplicialTestFuncs') && ~isempty(obj.Quality.SimplicialTestFuncs) && all(marg_testfunc_set)
        ...
        function inside = checkIfInsideQualitySpace(obj, pts, batch_size)
            % Check if the points are inside the quality space. Compute in batches if necessary to avoid excessive memory use.
            % Inputs:
            %   pts: matrix containing the input points, where each row corresponds to an input
        ...
        function vals = evaluateCostFunc(obj, marg_id, marg_inputs, quality_inputs, batch_size)
            % Evaluate the cost function between a marginal and the quality space. Computation is done in batches if necessary. 
            % Warning: this function does not check if the inputs are inside their corresponding domains. 
            % Inputs: 
            %   marg_id: index of the selected marginal
            %   marg_inputs: vector containing inputs in the support of the marginal
            %   quality_inputs: matrix where each row is an input in the quality space
            %   batch_size: number of inputs to be handled together (default is 1e4)
            % Output:
            ...
            assert(size(quality_inputs, 1) == input_num, 'input mis-specified');
            ...
            for batch_id = 1:batch_num
                ...
                batch_list = ((batch_id - 1) * batch_size + 1):min(batch_id * batch_size, input_num);
                batch_CPWA_inputs = marg_inputs(batch_list) - quality_inputs(batch_list, :) * cf.Weights;
                vals_cell{batch_id} = sum(min(max((batch_CPWA_inputs - cf.Knots(1:end - 1)') ./ cf.KnotDiff', 0), 1) ...
        ...
        function vals = evaluateSumOfCostFuncs(obj, marg_input_mat, quality_inputs, batch_size)
            % Evaluate the sum of cost functions. Computation is done in batches if necessary. Warning: this function does not check if 
            % the inputs are inside their corresponding domains. 
            % Inputs: 
            %   marg_input_mat: matrix where each row is an input and each column corresponds to a marginal
            %   quality_inputs: matrix where each row is an input in the quality space
            %   batch_size: number of inputs to be handled together (default is 1e4)
            % Output:
            %   vals: vector containing the computed cost values
            ...
            assert(size(quality_inputs, 1) == input_num, 'input mis-specified');
            assert(size(marg_input_mat, 2) == marg_num, 'input mis-specified');

            vals = zeros(input_num, 1);

            for marg_id = 1:marg_num
                vals = vals + obj.evaluateCostFunc(marg_id, marg_input_mat(:, marg_id), quality_inputs, batch_size);
            end
        end

        function setQualitySimplicialTestFuncs(obj, vertices, triangles, check_simpcover)
            % Initialize the test functions with respect to a simplicial cover and compute the integrals of the test functions with
            % respect to the probability measure. Warning: the function does not check if the union of the triangles in the simplicial 
            % cover equals the quality space given by the union of polytopes.
            % Inputs:
            ...
            obj.Quality.SimplicialTestFuncs.InvTransMat = vertcat(invtrans_cell{:});
        ...
        function [vals, inside] = evaluateQualitySimplicialTestFuncs(obj, pts, batch_size)
        ...
        function coup_cell = updateSimplicialTestFuncs(obj, quality_args, args_cell, fixed_index)
            ...
            for marg_id = 1:marg_num
                ...
                % store only the unique atoms in the discretized marginals and use their indices in the coupling with the quality space
                [old_coup_marg_atoms_cell{marg_id}, ~, uind_marg] = unique(round(input_atoms, 6), 'stable');
                old_coup_marg_probs_cell{marg_id} = accumarray(uind_marg, old_coup_probs_cell{marg_id});

                % store only the unique atoms in the quality space and use their indices in the coupling with the marginal
                [old_coup_q_atoms_cell{marg_id}, ~, uind_quality] = unique(round(quality_atoms, 6), 'rows', 'stable');
                old_coup_q_probs_cell{marg_id} = accumarray(uind_quality, old_coup_probs_cell{marg_id});
                
                old_coup_indices_cell{marg_id} = [uind_marg, uind_quality];
            ...
            fixed_dist_mat = pdist2(fixed_cand_q_atoms, obj.Quality.SimplicialTestFuncs.Vertices, 'euclidean');
            ...
            for marg_id = 1:marg_num
                new_atoms = obj.Marginals{marg_id}.SimplicialTestFuncs.Knots;
                new_probs = obj.Marginals{marg_id}.SimplicialTestFuncs.Integrals;

                % the cost function is the absolute distance
                dist_mat = abs(old_coup_marg_atoms_cell{marg_id} - new_atoms');

                % compute an optimal coupling between the original discrete marginal and the new discrete marginal discrete optimal
                % transport
                [marg_coup_atom_indices, marg_coup_probs] = discrete_OT(old_coup_marg_probs_cell{marg_id}, new_probs, dist_mat);

                % compute an optimal coupling between the measure on the quality space and the fixed measure on the quality space; 
                % this is to ensure that all measures on the quality space satisfy the constraints with respect to the test functions
                ...
                if size(q_dist_mat, 1) * size(q_dist_mat, 2) > 1e6
                    ...
                    OT = OTDiscrete(old_coup_q_probs_cell{marg_id}, fixed_meas_q_probs, q_dist_mat, cp_options);
                    ...
                    [q_coup_atom_indices, q_coup_probs] = discrete_OT(old_coup_q_probs_cell{marg_id}, fixed_meas_q_probs, q_dist_mat);
                end

                % perform discrete reassembly to get the new coupling; in this case, we need to reassemble both marginals in the 
                % coupling
                [coup_indices, coup_probs] = discrete_reassembly(old_coup_indices_cell{marg_id}, old_coup_probs_cell{marg_id}, ...
                    {marg_coup_atom_indices; q_coup_atom_indices}, {marg_coup_probs; q_coup_probs}, 1:2);
                coup_qualities = fixed_meas_q_atoms(coup_indices(:, 2), :);
                coup_inputs = new_atoms(coup_indices(:, 1), :);
                coup_atom_num = length(coup_probs);
                marg_testfunc_vals = sparse(1:coup_atom_num, coup_indices(:, 1), 1, coup_atom_num, length(new_atoms));
                quality_testfunc_vals = sparse(1:coup_atom_num, coup_indices(:, 2), 1, coup_atom_num, fixed_meas_q_atom_num);

                % compute the corresponding cost function values
                coup_costfunc_vals = obj.evaluateCostFunc(marg_id, coup_inputs, coup_qualities);
        ...
        function initializeSimplicialTestFuncs(obj)
            % Initialize some quantities related to the simplicial test functions of the marginals and the simplicial test functions
            ...
            % store the number of knots in the test functions for each marginal
            ...
            for marg_id = 1:marg_num
                ...
                obj.Storage.MargKnotNumList(marg_id) = length(marg.SimplicialTestFuncs.Knots);

                % note that the first test function for the marginal is removed for identification purposes
                obj.Storage.DeciVarMargTestFuncIndices{marg_id} = ind_counter + (1:obj.Storage.MargKnotNumList(marg_id) - 1)';
                ind_counter = ind_counter + obj.Storage.MargKnotNumList(marg_id) - 1;
            ...
            obj.Storage.TotalMargKnotNum = sum(obj.Storage.MargKnotNumList);

            if strcmp(obj.Options.global_formulation, 'INC_CC')
                % build the gurobi models for the global minimization problems; the one-dimensional CPWA functions are modeled by the 
                % incremental (INC) formulation and the two-dimensional CPWA functions are modeled by the convex combination (CC) 
                % formulation
                ...
                for marg_id = 1:marg_num
                    ...
                    % variables between 0 and 1 that are used to interpolate within the intervals for the cost function
                    zeta_indices = var_counter + (1:cf_knot_num - 1)';
                    var_counter = var_counter + cf_knot_num - 1;

                    % binary-valued variables for indicating the interval for the cost function
                    iota_indices = var_counter + (1:cf_knot_num - 2)';
                    var_counter = var_counter + cf_knot_num - 2;

                    % variables between 0 and 1 that are used to interpolate within the intervals for the parametric potential function
                    xi_indices = var_counter + (1:tf_knot_num - 1)';
                    var_counter = var_counter + tf_knot_num - 1;

                    % binary-valued variables for indicating the interval for the parametric potential function
                    eta_indices = var_counter + (1:tf_knot_num - 2)';
                    var_counter = var_counter + tf_knot_num - 2;

                    % variables between 0 and 1 that are used to represent a point in the quality space as a convex combination
                    beta_indices = var_counter + (1:quality_vert_num)';
                    var_counter = var_counter + quality_vert_num;

                    % binary-valued variables for indicating the triangle for the parametric transfer function on the quality space
                    chi_indices = var_counter + (1:tri_num)';
                    var_counter = var_counter + tri_num;

                    gl_model.modelsense = 'min';
                    gl_model.objcon = cf.Values(1);
                    gl_model.obj = [0; 0; 0; cf.ValueDiff; ...
                        zeros(cf_knot_num - 2, 1); ...
                        zeros(tf_knot_num - 1, 1); ...
                        zeros(tf_knot_num - 2, 1); ...
                        zeros(quality_vert_num, 1); zeros(tri_num, 1)];
                    gl_model.lb = [-inf; -inf; -inf; ...
                    ...
                    % constraint: iota_{t} - zeta_{t} <= 0 and constraint: iota_{t} - zeta_{t+1} >= 0
                    A_cf_ord_r = repelem((1:(cf_knot_num - 2) * 2)', 2, 1);
                    A_cf_ord_c_mat = [iota_indices'; zeta_indices(1:end - 1)'; iota_indices'; zeta_indices(2:end)'];
                    A_cf_ord_c = A_cf_ord_c_mat(:);
                    A_cf_ord_v = repmat([1; -1; 1; -1], cf_knot_num - 2, 1);
                    A_cf_ord = sparse(A_cf_ord_r, A_cf_ord_c, A_cf_ord_v, 2 * (cf_knot_num - 2), var_counter);
                    rhs_cf_ord = zeros(2 * (cf_knot_num - 2), 1);
                    sense_cf_ord = repmat(['<'; '>'], cf_knot_num - 2, 1);

                    % constraint linking zeta and the inputs
                    A_cf_link = sparse(ones(cf_knot_num - 1 + obj.Quality.Dim + 1, 1), ...
                        [zeta_indices; quality_indices; input_index], ...
                        [cf.KnotDiff; cf.Weights; -1], 1, var_counter);
                    rhs_cf_link = -cf.Knots(1);
                    sense_cf_link = '=';

                    % constraint: eta_{t} - xi_{t} <= 0 and constraint: eta_{t} - xi_{t+1} >= 0
                    A_tf_ord_r = repelem((1:(tf_knot_num - 2) * 2)', 2, 1);
                    A_tf_ord_c_mat = [eta_indices'; xi_indices(1:end - 1)'; eta_indices'; xi_indices(2:end)'];
                    A_tf_ord_c = A_tf_ord_c_mat(:);
                    A_tf_ord_v = repmat([1; -1; 1; -1], tf_knot_num - 2, 1);
                    A_tf_ord = sparse(A_tf_ord_r, A_tf_ord_c, A_tf_ord_v, 2 * (tf_knot_num - 2), var_counter);
                    rhs_tf_ord = zeros(2 * (tf_knot_num - 2), 1);
                    sense_tf_ord = repmat(['<'; '>'], tf_knot_num - 2, 1);

                    % constraint linking xi and the inputs
                    A_tf_link = sparse(ones(tf_knot_num - 1 + 1, 1), [xi_indices; input_index], [tf.KnotDiff; -1], 1, var_counter);
                    rhs_tf_link = -tf.Knots(1);
                    sense_tf_link = '=';

                    % constraint that the sum of all beta coefficients must be equal to 1
                    A_q_sum1 = sparse(ones(quality_vert_num, 1), beta_indices, ones(quality_vert_num, 1), 1, var_counter);
                    rhs_q_sum1 = 1;
                    sense_q_sum1 = '=';

                    % constraint that the sum of all chi variables must be equal to 1
                    A_q_sum2 = sparse(ones(tri_num, 1), chi_indices, ones(tri_num, 1), 1, var_counter);
                    rhs_q_sum2 = 1;
                    sense_q_sum2 = '=';
                    ...
                    for bit_id = 1:quality_vert_num
                        ...
                        tri_contain_list = find(any(quality_testfuncs.Triangles == bit_id, 2));
                        tri_contain_num = length(tri_contain_list);

                        A_q_id_r_cell{bit_id} = bit_id * ones(tri_contain_num + 1, 1);
                        A_q_id_c_cell{bit_id} = [beta_indices(bit_id); chi_indices(tri_contain_list)];
                        A_q_id_v_cell{bit_id} = [1; -ones(tri_contain_num, 1)];
                    end

                    A_q_id = sparse(vertcat(A_q_id_r_cell{:}), vertcat(A_q_id_c_cell{:}), vertcat(A_q_id_v_cell{:}), ...
                        quality_vert_num, var_counter);
                    rhs_q_id = zeros(quality_vert_num, 1);
                    sense_q_id = repmat('<', quality_vert_num, 1);

                    % constraint linking beta and the point in the quality space
                    A_q_link = sparse(repelem([1; 2], quality_vert_num + 1, 1), ...
                    ...
                    % indices in the decision vector to place the transformed coefficients of the parametric potential  functions
                    obj_tfcoef_r = repelem((1:tf_knot_num - 1)', 2, 1);
                    obj_tfcoef_c_mat = [1:tf_knot_num - 1; 2:tf_knot_num];
                    obj_tfcoef_v = repmat([1; -1], tf_knot_num - 1, 1);
                    gl_model.obj_tfcoef_mat = sparse(obj_tfcoef_r, obj_tfcoef_c_mat(:), obj_tfcoef_v, tf_knot_num - 1, tf_knot_num);
                    gl_model.obj_tfcoef_indices = xi_indices;

                    % indices in the decision vector to place the coefficients of the parametric transfer functions
                    gl_model.obj_qcoef_mat = -speye(quality_vert_num);
                    gl_model.obj_qcoef_indices = beta_indices;
                    
                    % matrix and intercept vector for directly extracting the values of the test functions for the marginal from the 
                    % results of the solver
                    gl_model.tfcoef_extract_mat = sparse([(1:tf_knot_num - 1)'; (2:tf_knot_num)'], ...
                        repmat(xi_indices, 2, 1), ...
                        [-ones(tf_knot_num - 1, 1); ...
                        ones(tf_knot_num - 1, 1)], ...
                        tf_knot_num, var_counter);
                    gl_model.tfcoef_extract_intercept = [1; zeros(tf_knot_num - 1, 1)];

                    % matrix for directly extracting the values of the test functions on the quality space from the result of the
                    % solver
                    gl_model.qcoef_extract_mat = sparse((1:quality_vert_num)', beta_indices, 1, quality_vert_num, var_counter);

                    obj.Storage.GlobalMinGurobiModels{marg_id} = gl_model;
                end
            elseif strcmp(obj.Options.global_formulation, 'LOG_DLOG')
                % build the gurobi models for the global minimization problems; the one-dimensional CPWA functions are modeled by the 
                % logarithmic convex combination (LOG) formulation and the two-dimensional CPWA functions are modeled by the 
                % logarithmic disaggregated convex combination (DLOG) formulation
                obj.Storage.GlobalMinGurobiModels = cell(marg_num, 1);
                ...
                % compute the list containing the vertex indices in the triangles
                ...
                for marg_id = 1:marg_num
                    ...
                    % compute the bisections of the knots in the cost function
                    cf_bit_length = ceil(log2(cf_knot_num - 1));
                    cf_bisect_cell = obj.compute1DBisection(cf_knot_num - 1);

                    % compute the bisections in the knots of the test functions
                    tf_bit_length = ceil(log2(tf_knot_num - 1));
                    tf_bisect_cell = obj.compute1DBisection(tf_knot_num - 1);
                    ...
                    % variables between 0 and 1 that are used to represent a point in a triangle in the quality space as a convex 
                    % combination
                    ...
                    gl_model.input_index = input_index;
                    gl_model.quality_indices = quality_indices;
                    gl_model.quality_convcombcoef_indices = vertcat(beta_indices{:});

                    % constraint that the sum of all zeta coefficients must be equal to 1
                    A_cf_sum = sparse(ones(cf_knot_num, 1), zeta_indices, ones(cf_knot_num, 1), 1, var_counter);
                    rhs_cf_sum = 1;
                    sense_cf_sum = '=';
                    ...
                    for bit_id = 1:cf_bit_length
                        ...
                        A_cf_id_r_cell{bit_id} = (bit_id - 1) * 2 + [ones(cf_bit0_num + 1, 1); 2 * ones(cf_bit1_num + 1, 1)];
                        A_cf_id_c_cell{bit_id} = [zeta_indices(cf_bit0_list); iota_indices(bit_id); ...
                            zeta_indices(cf_bit1_list); iota_indices(bit_id)];
                        A_cf_id_v_cell{bit_id} = [ones(cf_bit0_num, 1); -1; ones(cf_bit1_num, 1); 1];
                    end

                    A_cf_id = sparse(vertcat(A_cf_id_r_cell{:}), vertcat(A_cf_id_c_cell{:}), vertcat(A_cf_id_v_cell{:}), ...
                        2 * cf_bit_length, var_counter);
                    rhs_cf_id = repmat([0; 1], cf_bit_length, 1);
                    sense_cf_id = repmat('<', 2 * cf_bit_length, 1);

                    % constraint linking zeta and the inputs
                    A_cf_link = sparse(ones(cf_knot_num + obj.Quality.Dim + 1, 1), [zeta_indices; quality_indices; input_index], ...
                        [cf.Knots; cf.Weights; -1], 1, var_counter);
                    rhs_cf_link = 0;
                    sense_cf_link = '=';

                    % constraint that the sum of all xi coefficients must be equal to 1
                    A_tf_sum = sparse(ones(tf_knot_num, 1), xi_indices, ones(tf_knot_num, 1), 1, var_counter);
                    rhs_tf_sum = 1;
                    sense_tf_sum = '=';
                    ...
                    for bit_id = 1:tf_bit_length
                        ...
                        A_tf_id_r_cell{bit_id} = (bit_id - 1) * 2 + [ones(tf_bit0_num + 1, 1); 2 * ones(tf_bit1_num + 1, 1)];
                        A_tf_id_c_cell{bit_id} = [xi_indices(tf_bit0_list); eta_indices(bit_id); ...
                            xi_indices(tf_bit1_list); eta_indices(bit_id)];
                        A_tf_id_v_cell{bit_id} = [ones(tf_bit0_num, 1); -1; ones(tf_bit1_num, 1); 1];
                    end

                    A_tf_id = sparse(vertcat(A_tf_id_r_cell{:}), vertcat(A_tf_id_c_cell{:}), vertcat(A_tf_id_v_cell{:}), ...
                        2 * tf_bit_length, var_counter);
                    rhs_tf_id = repmat([0; 1], tf_bit_length, 1);
                    sense_tf_id = repmat('<', 2 * tf_bit_length, 1);

                    % constraint linking xi and the inputs
                    A_tf_link = sparse(ones(tf_knot_num + 1, 1), [xi_indices; input_index], [tf.Knots; -1], 1, var_counter);
                    rhs_tf_link = 0;
                    sense_tf_link = '=';

                    % constraint that the sum of all beta coefficients must be equal to 1
                    A_q_sum = sparse(ones(tri_num * 3, 1), vertcat(beta_indices{:}), ones(tri_num * 3, 1), 1, var_counter);
                    rhs_q_sum = 1;
                    sense_q_sum = '=';
                    ...
                    for bit_id = 1:tri_bit_length
                        % list of beta variables that are representing triangles with that specific bit being 0 or 1
                        tri_bit0_beta_list = vertcat(beta_indices{tri_bisect_cell{bit_id, 1}});
                        tri_bit0_beta_num = length(tri_bit0_beta_list);
                        tri_bit1_beta_list = vertcat(beta_indices{tri_bisect_cell{bit_id, 2}});
                        tri_bit1_beta_num = length(tri_bit1_beta_list);

                        A_q_id_r_cell{bit_id} = (bit_id - 1) * 2 + [ones(tri_bit1_beta_num + 1, 1); 2 * ones(tri_bit0_beta_num + 1, 1)];
                        A_q_id_c_cell{bit_id} = [tri_bit1_beta_list; chi_indices(bit_id); tri_bit0_beta_list; chi_indices(bit_id)];
                        A_q_id_v_cell{bit_id} = [ones(tri_bit1_beta_num, 1); -1; ones(tri_bit0_beta_num, 1); 1];
                    end
                    ...
                    % constraint linking beta and the point in the quality space
                    A_q_link = sparse(repelem([1; 2], tri_num * 3 + 1, 1), ...
                        [vertcat(beta_indices{:}); quality_indices(1); vertcat(beta_indices{:}); quality_indices(2)], ...
                        [quality_testfuncs.Vertices(tri_vert_list, 1); -1; quality_testfuncs.Vertices(tri_vert_list, 2); -1], ...
                        2, var_counter);
                    rhs_q_link = [0; 0];
                    sense_q_link = repmat('=', 2, 1);

                    gl_model.A = [A_cf_sum; A_cf_id; A_cf_link; A_tf_sum; A_tf_id; A_tf_link; A_q_sum; A_q_id; A_q_link];
                    gl_model.rhs = [rhs_cf_sum; rhs_cf_id; rhs_cf_link; rhs_tf_sum; rhs_tf_id; rhs_tf_link; ...
                        rhs_q_sum; rhs_q_id; rhs_q_link];
                    gl_model.sense = ...
                        [sense_cf_sum; sense_cf_id; sense_cf_link; sense_tf_sum; sense_tf_id; sense_tf_link; ...
                    ...
                    % indices in the decision vector to place the coefficients of the parametric transfer functions
                    gl_model.obj_qcoef_mat = sparse((1:tri_num * 3)', tri_vert_list, -1, tri_num * 3, quality_vert_num);
                    gl_model.obj_qcoef_indices = vertcat(beta_indices{:});

                    % matrix and intercept vector for directly extracting the values of the test functions for the marginal from the 
                    % results of the solver
                    gl_model.tfcoef_extract_mat = sparse((1:tf_knot_num)', xi_indices, 1, tf_knot_num, var_counter);
                    gl_model.tfcoef_extract_intercept = zeros(tf_knot_num, 1);

                    % matrix for directly extracting the values of the test functions on the quality space from the result of the
                    % solver
                    gl_model.qcoef_extract_mat = sparse(tri_vert_list, vertcat(beta_indices{:}), 1, quality_vert_num, var_counter);

                    obj.Storage.GlobalMinGurobiModels{marg_id} = gl_model;
                end
            else
                error('unknown formulation of the global minimization problem');
            end
        ...
        function coup_cell = generateDiscreteCoupling(obj)
            ...
            decivar_num = sum(marg_knot_num_list) + (quality_vert_num - 1) * marg_num;
            ...
            for marg_id = 1:marg_num
                ...
                % the integrals are placed into the objective vector; note that the first test function for the marginal is removed
                % for identification
                ...
                A_eq_cell{marg_id} = [sparse(quality_vert_num - 1, marg_knot_num), speye(quality_vert_num - 1)];
                ...
                [marg_grid, q_grid] = meshgrid(1:marg_knot_num, 1:quality_vert_num);
                marg_grid_indices = marg_grid(:);
                q_grid_indices = q_grid(:);
                A_marg_full = sparse(1:marg_knot_num * quality_vert_num, marg_grid_indices, 1, ...
                    marg_knot_num * quality_vert_num, marg_knot_num);
                A_quality_full = sparse(1:marg_knot_num * quality_vert_num, q_grid_indices, 1, ...
                ...
                % column of 1s
                A_ineq_cell{marg_id} = [sparse(ones(marg_knot_num * quality_vert_num, 1)), A_marg_full(:, 2:end), ...
                ...
                rhs_ineq_cell{marg_id} = obj.evaluateCostFunc(marg_id, marg_grid_pts, quality_grid_pts);

                constr_num_list(marg_id) = marg_knot_num * quality_vert_num;
            ...
            for marg_id = 1:marg_num
                ...
                coup_atom_num = length(coup_probs);
                coup_marg_points = marg_points_cell{marg_id}(pos_list, :);
                coup_quality_indices = quality_indices_cell{marg_id}(pos_list);
                coup_quality_points = quality_points_cell{marg_id}(pos_list, :);
                marg_testfunc_vals = sparse(1:coup_atom_num, marg_indices_cell{marg_id}(pos_list), 1, ...
                    coup_atom_num, marg_knot_num_list(marg_id));
                quality_testfunc_vals = sparse(1:coup_atom_num, coup_quality_indices, 1, coup_atom_num, quality_vert_num);
        ...
        function vals = evaluateOptParametricFunc(obj, marg_id, pts, batch_size)
            ...
            %   batch_size: the maximum number of input points to be handled at the same time in the vectorized procedure
            %   (default is 1e4)
            % Output:
        ...
        function vals_mat = evaluateOptTransferFuncs(obj, pts, ref_pt, batch_size)
            ...
            %   ref_pt: vector indicating a reference point where the transfer function will evaluate to 0 (default is the sample
            %   point from the quality space)
            %   check_inside: boolean indicating whether to check if the points are inside the quality space (default is true)
            %   batch_size: the maximum number of input points to be handled at the same time in the vectorized procedure
            %   (default is 1e4)
            % Output:
            ...
            assert(all(obj.checkIfInsideQualitySpace(pts)), 'some points are outside the quality space');
            ...
            for batch_id = 1:batch_num
                ...
                % only evaluate the first (marg_num - 1) transfer functions, the last one is determined by the balance condition
                ...
                for marg_id = 1:marg_num - 1
                    ...
                    % knots in the cost function for the input in the support of the marginal while the quality is fixed
                    shifted_cf_knots = obj.CostFuncs{marg_id}.Knots' + cur_pts * obj.CostFuncs{marg_id}.Weights;

                    shifted_cf_knots_vec = shifted_cf_knots(:);
                    shifted_cf_knots_inside = marg.checkIfInsideSupport(shifted_cf_knots_vec);
                    parfunc_vals = -inf(length(shifted_cf_knots_inside), 1);
                    parfunc_vals(shifted_cf_knots_inside) = marg.evaluateWeightedSumOfSimplicialTestFuncs( ...
                        shifted_cf_knots_vec(shifted_cf_knots_inside), coef);

                    % values attained at the knots of the cost functions
                    vals1 = obj.CostFuncs{marg_id}.Values' - reshape(parfunc_vals, cur_batch_size, ...
                        length(obj.CostFuncs{marg_id}.Knots));

                    % values attained at the knots of the test functions
                    vals2 = reshape(obj.evaluateCostFunc(marg_id, repelem(marg_knots, cur_batch_size, 1), ...
                        repmat(cur_pts, marg_knot_num, 1)), cur_batch_size, marg_knot_num) - coef';
                ...
                if batch_id == 1
                    ...
                    % shift the transfer functions to make the values vanish at the reference point, then remove the reference point
                ...
                % the last transfer function is obtained from the balance condition, the resulting transfer functions are guaranteed to 
                % add up to 0
                vals_cell{batch_id} = [vals_cell{batch_id}, -sum(vals_cell{batch_id}, 2)];
            end
        ...
        function samps = randSampleFromOptJointDistr(obj, samp_num, rand_stream)
            % Generate independent random sample from the joint distribution consisting of the coupling of the discretized marginals, 
            % the corresponding minimizers in the quality space, the coupled continuous marginals, and the corresponding continuous 
            ...
            %       DiscreteInputs: cell array containing the coordinates of samples from the coupling of the discretized marginals
            %       DiscreteQualities: matrix containing the coordinates of samples from the discrete measure on the quality space
            %       ContinuousInputs: cell array containing the coordinates of samples from the continuous marginals
            %       ContinuousQualities: matrix containing the coordinates of samples from the continuous measure on the quality space
            ...
            marg_num = length(obj.Marginals);
            [samps, atom_indices] = obj.doRandSampleFromPartialReassembly((1:marg_num)', samp_num, rand_stream);
            
            % get the corresponding samples from the discrete measure on the quality space by the atom indices
            samps.DiscreteQualities = obj.Runtime.DiscreteQualityCandidate.Atoms(atom_indices, :);

            % the samples from the continuous measure on the quality space are obtained by solving the minimization of the sum of cost 
            % functions over the quality space
            samps.ContinuousQualities = obj.computeMinCostOverQuality(horzcat(samps.ContinuousInputs{:}));
        end

        function coup = randSampleFromOptDiscreteCoupling(obj, marg_id, samp_num, rand_stream)
            ...
            %       Qualities: matrix containing the coordinates of samples from the discrete measure on the quality space
            %       AgentTypes: vector containing the coordinates of samples from the continuous marginal
            ...
            [samps, atom_indices] = obj.doRandSampleFromPartialReassembly(marg_id, samp_num, rand_stream);

            coup = struct;
            coup.AgentTypes = samps.ContinuousInputs{marg_id};

            % get the corresponding samples from the discrete measure on the quality space by the atom indices
            coup.Qualities = obj.Runtime.DiscreteQualityCandidate.Atoms(atom_indices, :);
        end

        function ql_samps = randSampleFromOptContinuousQualityDistr(obj, samp_num, rand_stream)
            ...
            %   rand_stream: RandStream object (default is RandStream.getGlobalStream)
            % Output:
            %   ql_samps: matrix containing the coordinates of samples from the discrete measure on the quality space
            ...
            samps = obj.randSampleFromOptJointDistr(samp_num, rand_stream);
            ql_samps = samps.ContinuousQualities;
        end

        function coup = randSampleFromOptContinuousCoupling(obj, marg_id, samp_num, rand_stream)
            ...
            % Output:
            %   coup: struct containing the following fields:
            %       Qualities: matrix containing the coordinates of samples from the continuous measure on the quality space
            %       AgentTypes: vector containing the coordinates of samples from the continuous marginal
        ...
        function [UB_disc, samps] = getMTUpperBoundDiscrete(obj, samp_num, rand_stream, batch_size)
            % Compute the upper bound for the matching for teams problem based on the discrete measure on the quality space. The bounds 
            % are approximated by Monte Carlo integration. Computation is done in batches if necessary.
            % Inputs: 
            %   samp_num: number of samples for Monte Carlo integration
            %   rand_stream: RandStream object (default is RandStream.getGlobalStream)
            %   batch_size: number of samples to be handled simultaneously during the computation of the sum of the cost functions
            %   (default is 1e4)
            % Output:
            ...
            UB_disc_list = obj.evaluateSumOfCostFuncs(horzcat(samps.ContinuousInputs{:}), samps.DiscreteQualities, batch_size);
        ...
        function [UB_disc, UB_cont, samps] = getMTUpperBounds(obj, samp_num, rand_stream, batch_size)
            % Compute two upper bounds for the matching for teams problem based on the discrete measure on the quality space and based
            % on the continuous measure on the quality space. The bounds are approximated by Monte Carlo integration. Computation is 
            ...
            UB_disc_list = obj.evaluateSumOfCostFuncs(horzcat(samps.ContinuousInputs{:}), samps.DiscreteQualities, batch_size);
            UB_cont_list = obj.evaluateSumOfCostFuncs(horzcat(samps.ContinuousInputs{:}), samps.ContinuousQualities, batch_size);
        ...
        function [UB_disc_list, samps] = getMTUpperBoundDiscreteWRepetition(obj, samp_num, rep_num, rand_stream, batch_size)
            % Compute the upper bound for the matching for teams problem based on the discrete measure on the quality space with 
            ...
            if ~isempty(obj.Options.log_file)
                ...
                fprintf(log_file, '--- Monte Carlo sampling (discrete measure) starts ---\n');
            end

            for rep_id = 1:rep_num
                [UB_disc_list(rep_id), samps] = obj.getMTUpperBoundDiscrete(samp_num, rand_stream, batch_size);

                % display output
                if obj.Options.display
                    fprintf('%s: Monte Carlo sampling (discrete measure) repetition %3d done\n', class(obj), rep_id);
                end

                % write log
                if ~isempty(obj.Options.log_file)
                    fprintf(log_file, '%s: Monte Carlo sampling (discrete measure) repetition %3d done\n', class(obj), rep_id);
                end
            end

            % close the log file
            if ~isempty(obj.Options.log_file)
                fprintf(log_file, '--- Monte Carlo sampling (discrete measure) ends ---\n\n');
                fclose(log_file);
            end
        end
    
        function [UB_disc_list, UB_cont_list, samps] = getMTUpperBoundsWRepetition(obj, samp_num, rep_num, rand_stream, batch_size)
            ...
            for rep_id = 1:rep_num
                [UB_disc_list(rep_id), UB_cont_list(rep_id), samps] = obj.getMTUpperBounds(samp_num, rand_stream, batch_size);
        ...
        function EB = getMTErrorBoundBasedOnOT(obj)
            ...
            % cost functions and the optimal transport distances between the marginals and their discretizations
            % Output:
            ...
            % make sure that the couplings between the candidate discrete measure and the discretized marginals are computed
            ...
            for marg_id = 1:marg_num
                ...
                [~, uind, mapping] = unique(round(marg_atoms, 6), 'stable');
                marg_atoms = marg_atoms(uind);
                marg_probs = accumarray(mapping, marg_probs, [length(marg_atoms), 1]);

                marg.setCoupledDiscreteMeasure(marg_atoms, marg_probs);
                EB = EB + costfunc.LipschitzConstQuality * obj.Runtime.DiscreteOTCosts(marg_id) ...
                    + costfunc.LipschitzConstMarg * marg.computeOTCost();
            end
        end
        ...
        function EB = getMTTheoreticalErrorBound(obj, LSIP_tolerance)
            ...
            for marg_id = 1:marg_num
                ...
                EB = EB + costfunc.LipschitzConstQuality * 2 * obj.Quality.SimplicialTestFuncs.MeshSize ...
                    + costfunc.LipschitzConstMarg * 2 * marg.SimplicialTestFuncs.MeshSize;
            end
        end
    end
    ...
    methods(Access = protected)
        ...
        function inside = doCheckIfInsideQualitySpace(obj, pts)
            ...
            check_model = obj.Quality.GurobiModel;
            lb_list = all(pts >= check_model.lb(1:obj.Quality.Dim)' - MatchTeam1DCPWA.INSIDE_TOLERANCE, 2);
            ub_list = all(pts <= check_model.ub(1:obj.Quality.Dim)' + MatchTeam1DCPWA.INSIDE_TOLERANCE, 2);

            if obj.Quality.AuxVarNum == 0
                % if the quality space is characterized by inequality and equality constraints only without any auxiliary variables, we 
                % can simply check whether these constraints are satisfied
                constr_diff =  pts * check_model.A' - check_model.rhs';

                ineq_list = all(constr_diff(:, check_model.sense == '<') <= MatchTeam1DCPWA.INSIDE_TOLERANCE, 2);
                eq_list = all(abs(constr_diff(:, check_model.sense == '=')) <= MatchTeam1DCPWA.INSIDE_TOLERANCE, 2);

                inside = lb_list & ub_list & ineq_list & eq_list;
            else
                % if there are auxiliary variables, we solve an LP problem to determine whether the point is inside the quality space
                ...
                rhs_mat = check_model.rhs - check_model.A(:, 1:obj.Quality.Dim) * rem_pts';
        ...
        function bisect_cell = compute1DBisection(obj, intv_num)
            ...
            if isempty(obj.Storage.ReflexiveBinarySequences{bit_len})
                ...
                for bit_id = 2:bit_len
                    ...
                    seq_mat(prev_row_num + (1:prev_row_num), end - bit_id + 1:end) ...
                        = [ones(prev_row_num, 1), flipud(seq_mat(1:prev_row_num, end - bit_id + 2:end))];
                end
        ...
        function [min_pts, min_vals] = computeMinCostOverQuality(obj, input_mat)
            % Solve the minimization of the sum of cost functions where the arguments in the support of marginals are given.
            % Inputs:
            %   input_mat: matrix where each row corresponds to an input and each column corresponds to a marginal
            % Outputs:
            %   min_pts: the computed minimizers stored in a matrix where each row contains a minimizer
            ...
            for input_id = 1:input_num
                model.rhs(model.rhs_input_indices) = obj.Quality.MinCostGurobiModel.rhs(model.rhs_input_indices) ...
                    + input_mat(input_id, :)';
        ...
        function prepareRuntime(obj)
            ...
            for marg_id = 1:marg_num
                ...
                obj.Runtime.CutQualities{marg_id} = zeros(0, obj.Quality.Dim);
            end
        end
        ...
        function [vals, inside] = doEvaluateQualitySimplicialTestFuncs(obj, pts)
        ...
        function model = generateInitialMinModel(obj)
            ...
            for marg_id = 1:marg_num
                model.A_ineq_cell{marg_id} = sparse(0, 1 + marg_knot_num_list(marg_id) - 1 + quality_vert_num - 1);
        ...
        function [min_lb, optimizers] = callGlobalMinOracle(obj, vec)
            ...
            model_cell = obj.Storage.GlobalMinGurobiModels;
            marg_LB_list = obj.Storage.MarginalLowerBounds;
            marg_UB_list = obj.Storage.MarginalUpperBounds;
            objective_const_list = zeros(marg_num, 1);
            marg_vert_vals_cell = cell(marg_num, 1);
            quality_vert_vals_cell = cell(marg_num, 1);
            new_cut_thres = obj.Options.new_cut_thres;

            for marg_id = 1:marg_num
                % disassemble the vector resulted from solving the LP problem; the first test function is added back to the list of 
                % test functions for the marginals; similarly the first test function is added back to the list of test functions on 
                % the quality space
                objective_const_list(marg_id) = vec(intecept_indices(marg_id));
                marg_vert_vals_cell{marg_id} = [0; vec(marg_tfuncs_indices{marg_id})];
                quality_vert_vals_cell{marg_id} = [0; vec(q_tfuncs_indices{marg_id})];
            end
            ...
            if obj.Options.global_parfor
                parfor marg_id = 1:marg_num
                    % compute a pool of approximate solutions
                    [pool_inputs, pool_qualities, pool_marg_testfuncs, ...
                        pool_quality_testfuncs, pool_vals, pool_costs, min_vals_list(marg_id)] ...
                        = MatchTeam1DCPWA.computeGlobalMinMILP(model_cell{marg_id}, gl_options, ...
                        quality_vert_vals_cell{marg_id}, marg_vert_vals_cell{marg_id}, objective_const_list(marg_id), ...
                        marg_LB_list(marg_id), marg_UB_list(marg_id));
                    pool_neg_list = pool_vals < -new_cut_thres;
                    optimizers{marg_id} = struct( ...
                        'inputs', pool_inputs(pool_neg_list), ...
                        'qualities', pool_qualities(pool_neg_list, :), ...
                        'marg_testfunc_vals', pool_marg_testfuncs(pool_neg_list, :), ...
                        'quality_testfunc_vals', pool_quality_testfuncs(pool_neg_list, :), ...
        ...
        function addConstraints(obj, optimizers)
            ...
            for marg_id = 1:marg_num
                ...
                if isfield(optimizers_i, 'marg_testfunc_vals')
                    marg_testfunc_vals = optimizers_i.marg_testfunc_vals;
                else
                    marg_testfunc_vals = obj.Marginals{marg_id}.evaluateSimplicialTestFuncs(optimizers_i.inputs);
                end

                if isfield(optimizers_i, 'quality_testfunc_vals')
                    quality_testfunc_vals = optimizers_i.quality_testfunc_vals;
                else
                    quality_testfunc_vals = obj.evaluateQualitySimplicialTestFuncs(optimizers_i.qualities);
                end

                % sanitize the output to avoid potential numerical issues; first, check for entries that are very close to 1, set them 
                % to 1 and set the remaining entries in those rows to 0; then, set entries that are close to 0 to 0
                [m_r, m_c, m_v] = find(marg_testfunc_vals);
                closeto1_list = abs(m_v - 1) < obj.Options.sanitation_threshold;
                m_v(closeto1_list) = 1;
                keep_list = ~(ismember(m_r, m_r(closeto1_list)) & ~closeto1_list) & m_v >= obj.Options.sanitation_threshold;
                marg_testfunc_vals = sparse(m_r(keep_list), m_c(keep_list), m_v(keep_list), ...
                    size(marg_testfunc_vals, 1), size(marg_testfunc_vals, 2));

                [q_r, q_c, q_v] = find(quality_testfunc_vals);
                closeto1_list = abs(q_v - 1) < obj.Options.sanitation_threshold;
                q_v(closeto1_list) = 1;
                keep_list = ~(ismember(q_r, q_r(closeto1_list)) & ~closeto1_list) & q_v >= obj.Options.sanitation_threshold;
                quality_testfunc_vals = sparse(q_r(keep_list), q_c(keep_list), q_v(keep_list), ...
                    size(quality_testfunc_vals, 1), size(quality_testfunc_vals, 2));

                % remove the first test function for identification, then prepend a column of 1 and add the values of the simplicial 
                % test functions on the quality space on the right
                A_new = [sparse(ones(constr_num, 1)), sparse(marg_testfunc_vals(:, 2:end)), sparse(quality_testfunc_vals(:, 2:end))];

                rhs_new = optimizers_i.costfunc_vals;
                closeto0_list = abs(rhs_new) < obj.Options.sanitation_threshold;
                ...
                obj.Runtime.CutInputs{marg_id} = [obj.Runtime.CutInputs{marg_id}; optimizers_i.inputs];
                obj.Runtime.CutQualities{marg_id} = [obj.Runtime.CutQualities{marg_id}; optimizers_i.qualities];
        ...
        function reduceConstraints(obj, result)
            ...
            for marg_id = 1:marg_num
                ...
                obj.Runtime.CutInputs{marg_id} = obj.Runtime.CutInputs{marg_id}(keep_list);
                obj.Runtime.CutQualities{marg_id} = obj.Runtime.CutQualities{marg_id}(keep_list, :);
        ...
        function primal_sol = buildPrimalSolution(obj, result, ~)
            ...
            for marg_id = 1:marg_num
                ...
                % similarly, add back the first test function on the quality space
        ...
        function dual_sol = buildDualSolution(obj, result)
            ...
            %   result: output of the gurobi LP solver
            % Output:
            %   dual_sol: the constructed discrete probability measure for the relaxed MMOT problem coupled with the corresponding
            ...
            for marg_id = 1:marg_num
                ...
                dual_sol{marg_id}.InputAtoms = obj.Runtime.CutInputs{marg_id}(pos_list);
                dual_sol{marg_id}.QualityAtoms = obj.Runtime.CutQualities{marg_id}(pos_list, :);
        ...
        function prepareDiscreteOT(obj)
            ...
            [cand_atoms, ~, cand_uind] = unique(round(cand.QualityAtoms, 6), 'rows', 'stable');
            cand_atom_num = size(cand_atoms, 1);
            cand_probs = accumarray(cand_uind, cand.Probabilities, [cand_atom_num, 1]);
            ...
            for marg_id = 1:marg_num
                dual_sol = obj.Runtime.DualSolution{marg_id};
                [marg_atoms, ~, m_uind] = unique(round(dual_sol.InputAtoms, 6), 'rows', 'stable');
                ...
                if marg_id == cand_id
                    ...
                    coup_cell{marg_id} = sparse(cand_uind, m_uind, cand.Probabilities, cand_atom_num, marg_atom_num);
                    discreteOT_cost_list(marg_id) = 0;

                    continue;
                end

                [quality_atoms, ~, q_uind] = unique(dual_sol.QualityAtoms, 'rows', 'stable');
                ...
                q2m_coup_mat = sparse(q_uind, m_uind, dual_sol.Probabilities, quality_atom_num, marg_atom_num);
                ...
                quality_probs = accumarray(q_uind, dual_sol.Probabilities, [quality_atom_num, 1]);
                ...
                if size(dist_mat, 1) * size(dist_mat, 2) > 1e6
                    ...
                    [coup_atoms, coup_probs, discreteOT_cost_list(marg_id)] = discrete_OT(cand_probs, quality_probs, dist_mat);
                end
                
                % construct the sparse matrix representing the coupling between the candidate discrete measure and this discrete
                % measure on the quality space
                q2q_coup_mat = sparse(coup_atoms(:, 1), coup_atoms(:, 2), coup_probs, cand_atom_num, quality_atom_num);
            ...
            obj.Runtime.DiscreteQualityCandidate = struct('Probabilities', cand_probs, 'Atoms', cand_atoms);
        ...
        function [samps, disc_atom_index_samps] = doRandSampleFromPartialReassembly(obj, marg_to_reassemble, samp_num, rand_stream)
            ...
            %       DiscreteInputs: cell array containing the coordinates of samples from the coupling of the discretized marginals
            %       ContinuousInputs: cell array containing the coordinates of samples from the continuous marginals (only those
            %       included in marg_to_reassmeble will be sampled)
            %   disc_atom_index_samps: vector containing the indices of atoms in the discrete coupling
            ...
            if ~isfield(obj.Runtime, 'CouplingDone') || ~obj.Runtime.CouplingDone
                % set the discrete measures that the marginals are coupled with
                for marg_id = 1:marg_num
                    disc_marg_atoms = disc_marg_atoms_cell{marg_id};
                    disc_marg_probs = sum(disc_couplings_cell{marg_id}, 1)';
            ...
            for marg_id = 1:marg_num
                ...
                for cand_atom_id = 1:cand_atom_num
                    ...
                    [cond_atoms, ~, cond_probs] = find(disc_couplings_cell{marg_id}(cand_atom_id, :)');
                    ...
                    if length(cond_probs) > 1
                        ...
                        samp_marg_atom_indices(fill_list) = randsample(rand_stream, cond_atoms, cond_samp_num, true, cond_probs);
                    elseif isscalar(cond_probs)
                        % set all samples to be equal to that atom
                        samp_marg_atom_indices(fill_list) = cond_atoms;
                    else
                        % rare edge case when the conditional distribution is undefined due to numerical inaccuracies
                        samp_marg_atom_indices(fill_list) = 1;
                    end
                end

                % store the coordinates of the samples with discrete marginals
                samps.DiscreteInputs{marg_id} = disc_marg_atoms(samp_marg_atom_indices);
                ...
                atom_num_list = accumarray(samp_marg_atom_indices, 1, [size(disc_marg_atoms, 1), 1]);

                % generate from the conditional distributions
                cont_samp_cell = marg.conditionalRandSample((1:size(disc_marg_atoms, 1))', atom_num_list, rand_stream);

                % fill in the coupled samples from the continuous marginals
                for atom_id = 1:length(atom_num_list)
                    samps.ContinuousInputs{marg_id}(samp_marg_atom_indices == atom_id, :) = cont_samp_cell{atom_id};
                end
            end
        end
    end
    ...
    methods(Access = protected, Static)
        function [pool_inputs, pool_qualities, pool_marg_testfuncs, pool_quality_testfuncs, pool_vals, pool_costs, LB] ...
                = computeGlobalMinMILP(model, gl_options, quality_vert_vals, marg_vals, objective_const, marg_LB, marg_UB)
            % Solve the global minimization problem via the mixed-integer linear programming (MILP) formulation; declared static to
            % allow better memory management when executed in parallel
            % Input:
            %   model: gurobi model (without setting the objective vector)
            %   gl_options: struct containing the gurobi MILP options
            %   quality_vert_vals: values of the simplicial test functions on the quality spaces at the vertices
            %   marg_vals: values of the simplicial test functions for the  marginals at the knots
            %   objective_const: constant part of the objective function
            %   marg_LB: lower bound for truncating the approximate minimizers
            %   marg_UB: upper bound for truncating the approximate minimizers
            % Outputs:
            %   pool_inputs: matrix containing the inputs in the support of the marginals from the approximate optimizers
            %   pool_qualities: matrix containing the quality part of the approximate optimizers
            %   pool_marg_testfuncs: sparse matrix containing the values of the simplicial test functions for the marginal evaluated at
            %   the resulting inputs
            %   pool_quality_testfuncs: sparse matrix containing the values of the simplicial test functions on the quality space 
            %   evaluated at the resulting quality points
            %   pool_vals: the corresponding objective values of the approximate optimizers
            %   pool_costs: the corresponding values of the sum of the cost functions evaluated at the approximate optimizers on the
            %   quality space
            %   LB: the best lower bound found throughout the algorithm

            % this vector has the coefficients of all test functions set to zero; thus it can be used to evaluate the sum of the cost
            ...
            model.obj(model.obj_tfcoef_indices) = model.obj_tfcoef_mat * marg_vals;
            model.objcon = model.objcon - objective_const;

            % fill in the coefficients in the parametric transfer function
            model.obj(model.obj_qcoef_indices) = model.obj_qcoef_mat * quality_vert_vals;
            ...
            pool_marg_testfuncs = pool_cuts * model.tfcoef_extract_mat' + model.tfcoef_extract_intercept';
            pool_quality_testfuncs = pool_cuts * model.qcoef_extract_mat';

            % make sure that the inputs are within their respective bounds (they may be slightly out of bound due to small numerical
            % inaccuracies)
            pool_inputs = min(max(pool_inputs, marg_LB), marg_UB);
