...
% if a single cell in the histogram has probability at least 0.1, it is
% regarded as an atom at the center of the cell
cont_atom_thres = 5e-3;

cell_area = (quality_hist_edge_x(2) - quality_hist_edge_x(1)) ...
    * (quality_hist_edge_y(2) - quality_hist_edge_y(1));
...
for test_id = 1:test_num
    ...
    a_lin = sub2ind([quality_hist_x_num, quality_hist_y_num], ...
        a_r, a_c);
    cont_atoms{test_id} = [quality_plot_hist_grid_x(a_lin), ...
        quality_plot_hist_grid_y(a_lin)];
...
ha = tight_subplot(2, test_num, [0.03, 0.01], ...
    [0.05, 0.01], [0.01, 0.005]);
theo_sparsity = zeros(test_num, 1);

for test_id = 1:test_num
    axes(ha(test_id));
    hold on;
    box on;
    bbc = bubblechart(disc_atoms{test_id}(:, 1), ...
        disc_atoms{test_id}(:, 2), ...
        disc_probs{test_id}, ...
        'red', ...
        'MarkerEdgeAlpha', 0.6, ...
        'MarkerFaceAlpha', 'flat', 'AlphaDataMapping', 'scaled');
    bbc.AlphaData = sort(disc_probs{test_id} .^ 0.25, 'descend');
    bubblelim([prob_min, prob_max * 2]);
    ...
    pc = pcolor(quality_plot_hist_grid_x, quality_plot_hist_grid_y, ...
        cont_densi{test_id});
    pc.FaceColor = 'interp';
    pc.EdgeColor = 'interp';
    ...
    bubblechart(cont_atoms{test_id}(:, 1), ...
        cont_atoms{test_id}(:, 2), ...
        cont_probs{test_id}, ...
        'red', 'MarkerFaceAlpha', 0.3, 'MarkerEdgeAlpha', 0.6);
    bubblelim([prob_min, prob_max * 2]);

    for atom_id = 1:length(cont_probs{test_id})
        text(cont_atoms{test_id}(atom_id, 1) - 0.12, ...
            cont_atoms{test_id}(atom_id, 2), ...
            sprintf('%.3f', cont_probs{test_id}(atom_id)), ...
    ...
    for marg_id = 1:marg_num
        ...
        theo_sparsity(test_id) = max(theo_sparsity(test_id), tf_num);
    end

    quality_testfuncs_num = size(quality_testfuncs_cell{test_id}{1}, 1) ...
        - 1;
    theo_sparsity(test_id) = theo_sparsity(test_id) ...
        + quality_testfuncs_num;
    xlabel(sprintf('(%d,%d)', marg_testfuncs_num_sum, ...
        quality_testfuncs_num), 'FontWeight', 'bold');

    fprintf(['test %d: theoretical sparsity = %d, ' ...
        'actual sparsity = %d\n'], ...
