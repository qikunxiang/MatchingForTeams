...
for trial_id = 1:trial_num
    ...
    input_file = load(input_file_path, 'testfuncs_cell', 'quality_testfuncs');

    for marg_id = 1:max(marg_list)
        marg_testfunc_num_mat(trial_id, marg_id) = length(input_file.testfuncs_cell{marg_id}{1});
    end

    quality_testfunc_num_list(trial_id) = size(input_file.quality_testfuncs{1}, 1);
end
...
for marg_test_id = 1:marg_test_num
    ...
    for trial_id = 1:trial_num
        ...
        if exist(result_file_path, 'file')
            ...
            RST.THEB(marg_test_id, trial_id) = result_file.MT_THEB;
            RST.TIME(marg_test_id, trial_id) = result_file.output.total_time;
