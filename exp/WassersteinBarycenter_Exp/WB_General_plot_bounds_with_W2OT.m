...
load(CONFIG.SAVEPATH_OUTPUTS_W2OTUB);
...
MT_W2OTUB2_err = zeros(test_num, 2);

for test_id = 1:test_num
    qq2 = quantile(MT_W2OTUB_cell{test_id}, [0.025, 0.975]);

    MT_W2OTUB2_err(test_id, 1) = MT_W2OTUB_mean_list(test_id) - qq2(1);
    MT_W2OTUB2_err(test_id, 2) = qq2(2) - MT_W2OTUB_mean_list(test_id);
end
...
handle_W2OTUB1 = plot(decivar_num_list, MT_W2OTUB_disc_list, ...
    'Marker', 'x', 'Color', 'cyan', 'LineStyle', ':', 'LineWidth', line_width);
handle_W2OTUB2 = plot(decivar_num_list, MT_W2OTUB_mean_list, ...
...
legend([handle_UB1, handle_UB2, handle_W2OTUB1, handle_W2OTUB2, handle_LB], ...
    {'$\hat{\alpha}_{\mathsf{MT}}^{\mathsf{UB}}$', ...
    '$\tilde{\alpha}_{\mathsf{MT}}^{\mathsf{UB}}$', ...
    '$\hat{\beta}_{\mathsf{WB}}^{\mathsf{UB}}$', ...
    '$\breve{\beta}_{\mathsf{WB}}^{\mathsf{UB}}$', ...
...
handle_W2OTcom1 = plot(decivar_num_list, MT_W2OTUB_disc_list - MT_LB_list, ...
    'Marker', 'none', 'Color', 'green', 'LineStyle', ':', 'LineWidth', line_width);
handle_W2OTcom2 = errorbar(decivar_num_list, MT_W2OTUB_mean_list - MT_LB_list, MT_W2OTUB2_err(:, 1), MT_W2OTUB2_err(:, 2), ...
    'Marker', 'none', 'Color', 'cyan', 'LineStyle', ':', 'LineWidth', line_width);
...
legend([handle_com1, handle_com2, handle_W2OTcom1, handle_W2OTcom2, handle_th], ...
    {'$\hat{\epsilon}_{\mathsf{sub}}$', ...
    '$\tilde{\epsilon}_{\mathsf{sub}}$', ...
    '$\hat{\xi}_{\mathsf{sub}}$', ...
    '$\breve{\xi}_{\mathsf{sub}}$', ...
...
bound_conservative_disc = MT_THEB_list ./ MT_W2OTdiff_disc_list;
bound_conservative = MT_THEB_list ./ MT_W2OTdiff_mean_list;
...
fprintf('The a priori upper bound is %d to %d times more conservative than the computed W_2-based bound 1.\n', ...
    round(min(bound_conservative_disc)), round(max(bound_conservative_disc)));
fprintf('The a priori upper bound is %d to %d times more conservative than the computed W_2-based bound 2.\n', ...
    round(min(bound_conservative)), round(max(bound_conservative)));
