...
for marg_id = 1:marg_num
    ...
    marg_dens_cell{marg_id} = reshape(Meas.densityFunction(marg_plot_grid_cell{marg_id}), plot_y_num, plot_x_num);
...
ha = tight_subplot(1, 6, [0, 0.015], [0.135, 0.005], [0.015, 0.04]);
...
plot(city, 'FaceColor', '#4DBEEE', 'EdgeColor', '#0072BD', 'LineWidth', 5);

rad = 0.15;
ang = linspace(0, 2 * pi, 200)';

for sta_id = 1:size(costfuncs.stations, 1)
...
plot(arr_ps_l, 'EdgeColor', '#77AC30', 'FaceColor', 'none', 'LineWidth', 2);
plot(arr_ps_r, 'EdgeColor', '#77AC30', 'FaceColor', '#77AC30', 'LineWidth', 2);
text(1.36, -0.9, 'N', 'FontWeight', 'bold', 'Color', '#77AC30', 'FontSize', 16);

set(gca, 'XLim', x_axis_lim);
set(gca, 'YLim', y_axis_lim);
set(gca, 'YTick', -3:1:2);
xlabel('train stations', 'FontSize', 15);

for marg_id = 1:marg_num
    axes(ha(marg_id + 1));
    hold on;
    plot_color = pcolor(marg_plot_grid_x_cell{marg_id}, marg_plot_grid_y_cell{marg_id}, marg_dens_cell{marg_id});
    ...
    if plot_testfuncs
        ...
        triplot(Meas.SimplicialTestFuncs.Triangles, Meas.SimplicialTestFuncs.Vertices(:, 1), Meas.SimplicialTestFuncs.Vertices(:, 2));
    end
    ...
    set(gca, 'YLim', y_axis_lim);
    set(gca, 'YTick', -3:1:2);
    xlabel(sprintf('$\\mu_{%d}$', marg_id), 'Interpreter', 'latex', 'FontSize', 17);
end

cb = colorbar(ha(end), 'manual');
cb.Position = [0.964, 0.135, 0.015, 0.86];