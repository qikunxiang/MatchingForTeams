...
load(input_file_path, 'testfunc_num_mat');
...
for test_id = 1:test_num
    tick_label_cell{test_id} = sprintf('%d', ...
...
legend([handle_UB1, handle_UB2, handle_LB], ...
    {'$\hat{\alpha}_{\mathsf{MMOT}}^{\mathsf{UB}}$', ...
    '$\tilde{\alpha}_{\mathsf{MMOT}}^{\mathsf{UB}}$', ...
    '$\alpha_{\mathsf{MMOT}}^{\mathsf{LB}}$'}, ...
    'Location', 'northeast', ...
    'Interpreter', 'latex', ...
    'FontSize', 13);
...
handle_com2 = errorbar(xx, MT_UB_mean_list(xx, 2) - MT_LB_list(xx), ...
    ...
    'Marker', 'none', 'Color', [214, 148, 0] / 255, ...
    'LineStyle', ':', 'LineWidth', line_width);
handle_ot = plot(xx, MT_OTEB_list(xx), ...
    'Marker', '^', 'Color', [35, 100, 52] / 255, ...
    'LineStyle', '-.', 'LineWidth', line_width);
...
legend([handle_com1, handle_com2, handle_ot, handle_th], ...
    {'$\hat{\epsilon}_{\mathsf{MMOT}}$', ...
    '$\tilde{\epsilon}_{\mathsf{MMOT}}$', ...
    '$\epsilon^\dagger_{W_1}$', ...
    '$\epsilon^\dagger_{\overline{\overline{W}}_1}$'}, ...
    'Location', 'northeast', ...
    'Interpreter', 'latex', ...
    'FontSize', 13);
